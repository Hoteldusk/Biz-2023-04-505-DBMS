...
-- 삭제, 제거 DELETE, DROP
-- DELETE : 데이터의 일부 요소를 삭제하기, 데이터의 구조등은 변화가 없다
...
-- 3. 생성상 사용자 계정에 권한(ROLL) 부여하기

-- login(접속만) 할 수 있는 권한 부여
...
-- DB 사용의 편의성을 위하여 오라클에서는 DBA 권한을 일반 사용자에게 부여할 수 있도록 하고있다

-- 이 권한은 실무(실제환경)에서는 절대 사용하면 안된다
-- 시스템의 중요한 데이터 사전을 제외한 모든 데이터를 핸들링(추가, 수정, 삭제, 제거 등)을 할 수 있는 권한이다
-- 다만 실습 환경에서만 사용하자

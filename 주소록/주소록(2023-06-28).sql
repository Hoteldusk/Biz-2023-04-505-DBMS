...
-- 엑셀파일에서 데이터 import (테이블 데이터에서 오른쪽마우스 데이터 임포트)
...
-- 모든 칼럼(*) 으로 조회를 하면, 실제 원하는 칼럼의 순서와 관계없이 데이터가 표현되기도 한다
SELECT * FROM tbl_address;

-- TABLE 의 모든 칼럼을 나열한 Projection
-- SELECT 명령에서 모든 칼럼(*)으로 조회하는 것 보다는 ALL Projection 으로 조히를 하는 것이, 조회 속도면에서 더 유리하다
-- 코딩(java .. 등등) 과 연동하여 사용할때는 가급적 All Project을 사용하는 것이 좋다
SELECT a_id, a_name, a_tel, a_addr FROM tbl_address;

-- A_ID가 A0010 이하인 데이터들만 보여달라
-- 일반적인 코딩 문법과 달리 SQL 에서는 문자열형 칼럼 데이터에 대하여 부등호 연산이 가능하다
-- 전체 데이터 중에서 특정 조건을 부여하여 조건에 맞는 데이터만 골라내는 명령 : Selection
SELECT * FROM tbl_address WHERE a_id <= 'A0010';

/*
Relataion Algebra(관계 대수)
...
SELECT SUBSTR(a_tel, 5) FROM tbl_address ORDER BY a_tel;
...
/*
SQL 에서 WHERE 등 조건을 부여하여 SELECTION 을 할때 특히 다중 조건을 부여할떄,
조건문의 순서에 따라 SELECT 결과의 효율이 달라질 수 있다
조금 더 깊이 생각하여 효율적인 SQL 명령문을 고민해 보자

1. 첫번째 SELECTION 먼저 범위를 제한하여 개수를 제한하고 전화번호를 찾는다
2. 두번째 SELECTION 전체 범위에서 전화번호를 찾고 그중에서 범위를 제한하여 개수를 제한한다
*/

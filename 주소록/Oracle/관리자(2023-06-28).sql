...
생성된 TABLESPACE 와 연결하는 사용자를 생성하고, 사용자(user)를 통하여 데이터에 접근한다
*/

-- 사용자생성
-- 오라클 21c(12c 이상) 에서는 사용자 등록을 하기 전에 SCRIPT 실행을 ON 해야한다
-- 오라클 12c 이상에서는 사용자ID c##아이디 형식으로 생성해야 한다
-- 사용자 ID 를 관리하기가 상당히 불편하다
-- 이 작업을 선행하지 않으면 사용자 ID와 TABLESAPCE를 연동할 수 없다
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

/*
addr 이라는 사용자ID를 등록하고
비밀번호는 12341234 fhgksek
또한 addr 사용자와 addrDB TABLESAPCE를 서로 연동한다
*/
...
/*
새로 생성한 사용자에게 DB접근(CRUD)할 수 있는 권한 부여
권한을 부여할때 로그인, CRUD 하기, TABLE 생성하기 등의 권한을 세부적으로 부여해야 하는데,
학습하는 상황에서 세부적인 권한부여는 다소 불편함이 있어서
DBA 의 권한을 부여한다

Oracle 에서 DBA의 권한은 SYSDBA 보다 다소 제한된 권한이다
System에 직접 접근하는 것은 금지하고, 그 외 권한을 모두 부여한다
*/

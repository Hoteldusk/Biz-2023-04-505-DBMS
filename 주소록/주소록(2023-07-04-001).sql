...
제1 정규화가 된 취미 데이터는 취미1, 취미2, 취미3 칼럼이 만들어진 table이다
...
Table의 칼럼에 null 값이 많은 것은 별로 좋은 현상이 아니다
*/
...
제 2정규화가 되어 있는 tbl_addr_hobby TABLE 에는
주소록의 ID(hb_aid) 와 취미 이름(hb_name)이 저장되어있다
hb_aid 칼럼은 주소 테이블과 연계되어 있지만
hb_name 칼럼은 평볌한 문자열로 구성되어있다
...
-- 3. 제3정규화 데이터 import
-- 주의! 기존 제2정규화 데이터를 엑셀로 보내서 제3정규화 데이터가 완료된 후 다음을 진행할 것
...
SELECT * FROM tbl_addr_hobby ORDER BY ah_seq;

-- TABLE JOIN
/*
정규화(2,3 정규화) 된 Table의 정보를 Select(조회)하면 단순히 1개의 Table 만으로는
어떤 정보를 보여주고 있는지 알 수가 없다
2,3 정규화 된 Table 은 필연적으로 Table 간의 Join이 되어야만 정보를 명확히 알 수 있다
JOIN은 2개 이상의 Table을 서로 연계하여 필요한 정보를 확인하는 것이다
*/
SELECT * FROM tbl_hobby, tbl_address;

-- tbl_addr_hobby * tbl_address
-- 카티션곱
SELECT COUNT(*) FROM tbl_hobby, tbl_address;

-- addr_hobby Releation 과 address Entity 테이블을 나열하되
-- 두테이블의 칼럼 ah_aid 와 a_id 의 데이터가 일치한 정보만 나열하라
-- EQ JOIN
-- EQ JOIN 은 모든 JOIN 방식중에 가장 빠르다. 단 참조 무결성 제약조건이 완벽해야 한다
-- EQ JOIN 은 연계하는 Table 간에 참조무결성 제약조건이 완벽할떄는
...
-- LEFT JOIN
-- JOIN 을 실행하는 Table 간에 참조무결성이 의심스럽거나 위배될지 모른다는 전재하에
-- 데이터를 확인하는 JOIN

-- FROM [table] LEFT JOIN [table2]
-- 왼쪽(LEFT) 의 table 데이터는 무조건 SELECT 하고
-- 오른쪽의 table2 데이터는 ON 에서 설정하는 조건에 맞는 것만 가져오기
-- 오른쪽의 table2 에 조건에 맞는 데이터가 있으면 보여주고 없으면 null 로 표현
...
-- 주소취미 Relataion 과 주소 Entity 간의 참조무결성 검사
...
-- view 의 생성
-- 물리적 Table 에 SQL 을 적용하여 작성하였더니 다소 복잡한 SQL이 완성되었다
...
FOREIGN KEY (ah_hbcode)  -- tbl_addr_hobby 의 ah_abcode 칼럼을

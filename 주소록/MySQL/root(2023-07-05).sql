-- 이 화면은 root 화면
/*
MySQL 은 DataBase 라는 Scheme 를 통하여 물리적인 DB를 관리한다
Oracle 은 TableSpace 라는 물리적 저장소와 연결한 사용자를 통하여
	DB를 관리한다
    
현업에서 가장 많이 사용하는 2가지의 DBMS 를 서로 비교하므로서
데이터베이스에 대한 이론을 정리해보기 바란다

Oracle 에서는 사용자가 Scheme 가 되며 MySQL 사용자는 DBMS, 물리적저장소 등에 접근하는 권한을 가진 사용자 개념이다

*/
-- 프로젝트 시작을 하기위하여 Database를 생성
...
/*
BIG INT type 의 PK 칼럼
	보통 기본 데이터로 PK를 생성할 수 없을 경우
	별도의 칼럼을 생성하고 그 칼럼에 일련변호 속성을 부여하여 PK로 만든다
    이때 PK 의 값은 무한히 커질 수 있으므로
    INT 형보다 상당히 큰 BIGINT type 으로 설정한다
    
AUTO_INCREMENT 속성
	MYSQL 에서는 일련변호 칼럼에 대하여 자동값 증가 생성 속성이 있다
    PK 칼럼에서만 이 속성을 부여할 수 있고,
    이 속성을 가진 PK 칼럼은 Insert 가 수행 될 때
...
SELECT * FROM tbl_hobby ORDER BY hb_code;
DELETE FROM tbl_hobby
WHERE hb_code = '';

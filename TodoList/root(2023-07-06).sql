...
My SQL 프로젝트 시작
1. database 생성
2. 사용자 등록(DB 서버가 Application 서버와 같은 동일한 운영체제에 ㅆ을떄는 사용자 등록이 선택사항이다
...
-- 사용자 등록
-- 사용자 등록을 할떼 접근할 수 있는 범위를 설정해야 한다
...
보안상 가장 위험한 사용자 생성
만약 이 사용자가 자신의 id 와 비밀번호를 소홀히 하여 누군가에게 노출된다면, 이 id 와 비밀번호를 통하여
...
-- DB 생성, Table 생성 등을 수행하려면 권한을 부여해야 한다
-- GRANT DBA TO user;
...
-- 네트워크를 통하여 접근한 todo 사용자 에게 
-- todoDB에 대하여 모든 권한을 부여하기
GRANT ALL PRIVILEGES ON tododb.* TO 'todo'@'192.168.4.%';

-- todoDB Database 의 tbl_todoList Table 만 접근하는 권한 부여
